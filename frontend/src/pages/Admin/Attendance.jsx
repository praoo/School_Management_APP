  ...
  const handleSubmit = async () => {
    try {
      const formattedData = attendanceData.map(({ id, status }) => ({
        student: id, // Ensure this is the ObjectId
        status,
      }));
        ...
        <AttendanceContent>
          ...
          <AttendanceList>
            {students.map((student) => (
              <AttendanceItem key={student.id}> {/* Ensure the key is unique */}
                <StudentName>{student.name}</StudentName>
                <RadioLabel>
                  <input
                    type="radio"
                    name={`attendance-${student.id}`}
                    checked={attendanceData.find((data) => data.id === student.id)?.status === 'Present'}
                ...
                <RadioLabel>
                  <input
                    type="radio"
                    name={`attendance-${student.id}`}
                    checked={attendanceData.find((data) => data.id === student.id)?.status === 'Absent'}
                ...
                <RadioLabel>
                  <input
                    type="radio"
                    name={`attendance-${student.id}`}
                    checked={attendanceData.find((data) => data.id === student.id)?.status === 'Absent with apology'}
